...
def calculate_demographic_data(print_data=True):
    ...
    # Convert 'hours-per-week' and 'age' to numeric types (integers) to avoid issues
    ...
    race_count = race_count.reindex(race_order).fillna(0).astype(int)
    ...
    rich_percentage = 0 if num_min_workers == 0 else round((df[(df['hours-per-week'] == min_work_hours) & (df['salary'] == '>50K')].shape[0] / num_min_workers) * 100, 1)
